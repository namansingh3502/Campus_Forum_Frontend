...
export default function ChannelSelect(props) {
  ...
  function addChannel(newChannel){
    for( let i = 0; i < selectedChannels.length; i++ ){
      if(newChannel.id === selectedChannels[i].id){ return }
    }
    ...
    <div className="min-w-full">
      <Combobox value={selected} onChange={setSelected}>
        <div className={"px-4 w-full"}>
          ...
          <div className={"my-4 flex flex-wrap text-white max-h-40 overflow-auto"}>
            {selectedChannels.map((channel)=>{
              return(
                <div
                  className={"bg-blue-600 bg-opacity-70 rounded-xl py-1 px-2 m-1 inline-block flex items-center max-w-fit text-sm"}
                  key={channel.id}
                >
                  ...
                  <Combobox.Option
                    ...
                    className={({ active }) =>
                      `select-none relative py-1 w-full px-4 ${
                        active ? 'text-white bg-blue-500 bg-opacity-70' : 'text-gray-100'
                      }`
                    }
                    value={channel}
                  >
