...
import LikeDetails from "./User_Reaction/likeDetails";
import axios from "axios";
import CommentModal from "./User_Reaction/commentModal";
  ...
  return (
    <div>
      <LikeDetails
        Liked={postLiked}
        UserLiked={userLiked}
        comment_count={props.comments_count}
        showCommentModal={()=>showCommentModal(true)}
        ...
        >
          <div className={"flex items-center justify-center"}>
            <svg
              ...
              className={ postLiked ? "h-7 w-7" : "h-6 w-6"}
              fill= { postLiked ? "#197dff" : "none" }
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={ postLiked ? "1" : "2" }
            >
              <path
                strokeLinecap="round"
